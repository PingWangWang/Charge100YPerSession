...
- **拍照辅助**：填写信息页的 3 个「上传照片」由你手动点击，弹出的二次确认「拍照」按钮由规则自动点击；相机内拍照、预览「完成」、上传照片均手动，保留你的取景与核对主动权。
...
- 定位选点：点击「重新选择」→ 搜索关键词如「元熙樾府」→ 选第一条 → 点击「完成」
- 拍照辅助：手动点击 3 个「上传照片」（侧前方/侧后方/左方等）；每个弹出的二次确认「拍照」按钮由规则自动点击；相机内拍照、预览「完成」、上传照片均手动
